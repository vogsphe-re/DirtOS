/// Home Assistant MQTT Publisher
///
/// When the DirtOS `home_assistant` integration is enabled and MQTT settings
/// are configured, this service:
///
...
/// 4. Re-publishes updated state on a configurable interval.
///
...
use crate::db::{integrations, models::IntegrationProvider};
...
pub async fn start(_app: AppHandle, pool: SqlitePool) {
    loop {
        match try_run(&pool).await {
            Ok(()) => {
                tracing::debug!("HA publisher: session ended, will check again in 60s");
            }
            Err(e) => {
...
async fn try_run(pool: &SqlitePool) -> Result<(), String> {
    ...
    if let Err(e) = publish_all(&client, pool, &prefix, &instance_id).await {
        tracing::warn!("HA publisher: initial publish error: {}", e);
    }
    ...
    loop {
        ...
        match tokio::time::timeout(timeout, event_loop.poll()).await {
            Ok(Ok(Event::Incoming(Packet::Publish(msg)))) => {
                if let Err(e) =
                    handle_command(pool, &msg.topic, &msg.payload, &prefix, &instance_id).await
                {
            ...
            Err(_) => {} // poll timeout, continue
        }

        if last_publish.elapsed() >= Duration::from_secs(interval_secs) {
            if let Err(e) = publish_all(&client, pool, &prefix, &instance_id).await {
                tracing::warn!("HA publisher: periodic publish error: {}", e);
            }
            last_publish = std::time::Instant::now();
        }

        // Stop if integration was disabled
        let configs = integrations::list_integration_configs(pool)
            .await
            .map_err(|e| e.to_string())?;
...
async fn publish_all(
    ...
    // Environments / gardens
    let envs: Vec<(i64, String)> =
        sqlx::query_as("SELECT id, name FROM environments ORDER BY name ASC")
            .fetch_all(pool)
            .await
            .map_err(|e| e.to_string())?;

    for (env_id, env_name) in &envs {
        publish_garden(client, pool, *env_id, env_name, prefix, instance_id).await?;
    }

    // Sensors
    let sensors: Vec<(i64, String, String, Option<i64>, i64)> = sqlx::query_as(
        "SELECT id, name, type, environment_id, is_active FROM sensors WHERE is_active = 1",
    )
    .fetch_all(pool)
    .await
    .map_err(|e| e.to_string())?;

    for (sensor_id, name, stype, env_id_opt, is_active) in &sensors {
        let env_name = env_id_opt
            .and_then(|eid| envs.iter().find(|(id, _)| *id == eid))
            .map(|(_, n)| n.as_str());
        publish_sensor(
            client,
            pool,
    ...
    // Plants
    let plants: Vec<(i64, String, String, i64)> = sqlx::query_as(
        "SELECT id, name, status, environment_id FROM plants ORDER BY environment_id, name",
    )
    .fetch_all(pool)
    .await
    .map_err(|e| e.to_string())?;

    for (plant_id, name, status, env_id) in &plants {
        let env_name = envs
            .iter()
            .find(|(id, _)| *id == *env_id)
            .map(|(_, n)| n.as_str());
        publish_plant(
            client, *plant_id, name, status, *env_id, env_name, prefix, instance_id,
        )
        .await?;
    }

    // Open issues as binary sensors
    let issues: Vec<(i64, String, String, String, Option<i64>)> = sqlx::query_as(
        "SELECT id, title, severity, status, environment_id FROM issues WHERE status = 'open'",
    )
    .fetch_all(pool)
    .await
    .map_err(|e| e.to_string())?;

    for (issue_id, title, severity, status, env_id_opt) in &issues {
        let env_name = env_id_opt
            .and_then(|eid| envs.iter().find(|(id, _)| *id == eid))
            .map(|(_, n)| n.as_str());
        publish_issue(
    ...
    tracing::info!(
        "HA publisher: published {} gardens, {} sensors, {} plants, {} issues",
        envs.len(),
        sensors.len(),
        plants.len(),
        issues.len()
...
async fn publish_garden(
    ...
    let plant_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM plants WHERE environment_id = $1")
            .bind(env_id)
            .fetch_one(pool)
            .await
            .unwrap_or(0);
    ...
    let config_topic = format!("{prefix}/{instance_id}/garden/{env_id}/config");
    client
        .publish(
            &config_topic,
            QoS::AtLeastOnce,
    ...
    let state_topic = format!("{prefix}/{instance_id}/garden/{env_id}/state");
    client
        .publish(
            &state_topic,
            QoS::AtLeastOnce,
...
async fn publish_sensor(
    ...
    let uid = format!("dirtos_{instance_id}_sensor_{sensor_id}");
    let avail_topic = format!("{prefix}/{instance_id}/availability");
    let state_topic = format!("{prefix}/{instance_id}/sensor/{sensor_id}/state");

    let device = if let Some(eid) = env_id {
        json!({
            "identifiers": [format!("dirtos_{instance_id}_garden_{eid}")],
    ...
    let config_topic = format!("{prefix}/{instance_id}/sensor/{sensor_id}/config");
    client
        .publish(
            &config_topic,
            QoS::AtLeastOnce,
    ...
    let reading: Option<(f64, Option<String>, String)> = sqlx::query_as(
        "SELECT value, unit, recorded_at FROM sensor_readings \
    ...
    if let Some((value, unit, recorded_at)) = reading {
        let state_payload = json!({
            "value": value,
            "unit": unit,
...
async fn publish_plant(
    ...
    let state_topic = format!("{prefix}/{instance_id}/plant/{plant_id}/state");
    client
        .publish(
            &state_topic,
            QoS::AtLeastOnce,
...
async fn publish_issue(
    ...
    let config_topic = format!("{prefix}/{instance_id}/issue/{issue_id}/config");
    client
        .publish(
            &config_topic,
            QoS::AtLeastOnce,
    ...
    let state_payload = json!({ "status": status });
    let state_topic = format!("{prefix}/{instance_id}/issue/{issue_id}/state");
    client
        .publish(
            &state_topic,
            QoS::AtLeastOnce,
...
async fn handle_command(
    ...
    if parts.last() == Some(&"sync") {
        tracing::info!("HA publisher: sync command received from HA");
        return Ok(());
    }
        ...
        && parts.last() == Some(&"set_active")
    {
        ...
        sqlx::query(
            "UPDATE sensors SET is_active = $1, updated_at = datetime('now') WHERE id = $2",
        )
        .bind(active)
        .bind(sensor_id)
        .execute(pool)
        ...
        tracing::info!(
            "HA publisher: sensor {} set is_active = {}",
            sensor_id,
            active
        );
        return Ok(());
    ...
    if parts.len() >= 5 && parts.get(parts.len() - 2) == Some(&"record_reading") {
        ...
        tracing::info!(
            "HA publisher: recorded reading {} for sensor {} via HA command",
            value,
            sensor_id
        );
        return Ok(());
    ...
    tracing::debug!(
        "HA publisher: unhandled command topic: {} (prefix={}, iid={})",
        topic,
        prefix,
        instance_id
    );
...
async fn publish_all(
    ...
    let environments = db::environments::list(pool)
        .await
        .map_err(|e| e.to_string())?;
    ...
    let plants = db::plants::list_all(pool)
        .await
        .map_err(|e| e.to_string())?;
